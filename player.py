...
class Player(Character):
    ...
    def __init__(self, images, sounds, i, genome):
        ...
        self.pos = (ScreenConfig.width // 2, ScreenConfig.height - ScreenConfig.y_offset)
    ...
    def shoot(self, bubble_images):
        ...
        Bubble(bubble_images, self.sounds, self.dir, (x, y), speed, i=self.i)
    ...
    def check_bubble_collision(self):
        ...
        if player_bubble:
            try:
                bubbles = player_bubble[self]
            except:
                return
            for bubble in bubbles:
                if bubble.i != self.i: continue
    ...
    def check_enemy_collision(self):
        if self.life > 0:
            if 0 < self.damaged_delay < ScreenConfig.player_damaged_delay:
    ...
    def _check_enemy_collision(self):
        enemy = pygame.sprite.spritecollideany(self, Enemy.group)
        if enemy and enemy.i == self.i:
            if not enemy.is_dead:
                if self.life > 0:
                    self.sounds['damaged'].play()
                    Boom(self.images['boom'], self.pos, i=self.i)
                    self.life -= 1
                    if self.life > 0:
                        self.damaged_delay = 1
                    elif not DeadPlayer.group:
                        # DeadPlayer(self.images['dead'], self.dir, self.pos, i=self.i)
                        pass
...
class DeadPlayer(Character):
    ...
    def __init__(self, image, dir, pos, i):
